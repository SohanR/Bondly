...
const Posts = (props) => {
  ...
  const [values, setValues] = useState({
    LikePost: checkLike(props.post.likes),
  ...
  const clickLike = () => {
    let callApi = values.LikePost ? UnlikePost : LikePost;

    callApi({ userId: user1.id }, { t: jwt.token }, props.post._id).then(
      (data) => {
        if (data) {
          setValues({
            ...values,
            LikePost: !values.LikePost,
  ...
  const deletePost = () => {
    removePost({ postId: props.post._id }, { t: jwt.token }).then((data) => {
    ...
    <div>
      <section className="posts border border_radius border-secondary mt-3  overflow-hidden p-0 white border position-relative">
        ...
        <h5 className="mt-0 ml-1">
          <p className="fff">
            {caption}
            {props.post.caption && props.post.caption.length > 300 ? (
              <a
                onClick={() => {
